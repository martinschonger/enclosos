...
classdef RefData < handle
    ...
    methods
        ...
        function plt_objs = plot_lines_presentation(obj, dim1, dim2, pltopts_equi, pltopts_traj, plot_velocities, plot_lines)
            arguments
                obj RefData;
                dim1(1, 1) {mustBeInteger} = 1;
                dim2(1, 1) {mustBeInteger} = 2;
            ...
            plt_objs.equilibrium = scatter(obj.Target(dim1), obj.Target(dim2), 40, 'o', 'markeredgecolor', 'black', 'markerfacecolor', 'black', 'linewidth', 1, 'displayname', '$\xi^*$', pltopts_equi{:});
            hold on;
            if plot_lines
                tmp_xiref = obj.getCellArrs();
                for n = 1:3
        ...
        function plt_objs = plot_presentation(obj, dim1, dim2, pltopts_traj, plot_velocities, plot_lines)
            arguments
                obj RefData;
                dim1(1, 1) {mustBeInteger} = 1;
                dim2(1, 1) {mustBeInteger} = 2;
            ...
            tmp_xiref = obj.getCellArrs();
            for n = 1:1
                plt_objs.trajectories{n} = scatter(tmp_xiref{n}(dim1, tmp_idxs), tmp_xiref{n}(dim2, tmp_idxs), 12, 'o', 'markeredgecolor', '#3392ff', 'markerfacecolor', '#3392ff', 'linewidth', 1, 'displayname', '$\xi^{\mathrm{ref}}$', pltopts_traj{:});
                if n > 1
                    set(plt_objs.trajectories{n}, 'handlevisibility', 'off');
                end
                hold on;
            ...
            plt_objs.velocities = quiver(obj.Data(dim1, tmp_idxs), obj.Data(dim2, tmp_idxs), obj.Data(obj.M+dim1, tmp_idxs) ./ 4, obj.Data(obj.M+dim2, tmp_idxs) ./ 4, 0, 'color', 'black', 'linewidth', 0.5, 'displayname', '$\dot{\xi}^{\mathrm{ref}}$');
            % tmp_idxs = tmp_idxs + 100;
            % plt_objs.velocities = quiver(obj.Data(dim1, tmp_idxs), obj.Data(dim2, tmp_idxs), obj.Data(obj.M+dim1, tmp_idxs), obj.Data(obj.M+dim2, tmp_idxs), 'color', 'black', 'linewidth', 0.5, 'displayname', '$\dot{\xi}^{\mathrm{ref}}$');
            set(plt_objs.velocities, 'linewidth', 1);
            set(plt_objs.velocities, 'color', '#3392ff');
        end
